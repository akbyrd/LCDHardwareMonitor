...
CTriangleRenderer::Init(IDirect3D9 *pD3D, IDirect3D9Ex *pD3DEx, HWND hwnd, UINT uAdapter)
{
	...
	IFC(m_pd3dDevice->SetTransform(D3DTS_VIEW, (D3DMATRIX*) &matView));
	matProj = XMMatrixPerspectiveFovLH(XM_PI / 4, 1.0f, 1.0f, 100.0f);
	IFC(m_pd3dDevice->SetTransform(D3DTS_PROJECTION, (D3DMATRIX*) &matProj));
...
CTriangleRenderer::Render()
{
	...
	FLOAT fAngle = iTime * (2.0f * XM_PI) / 1000.0f;
	matWorld = XMMatrixRotationY(fAngle);
	IFC(m_pd3dDevice->SetTransform(D3DTS_WORLD, (D3DMATRIX*) &matWorld));
