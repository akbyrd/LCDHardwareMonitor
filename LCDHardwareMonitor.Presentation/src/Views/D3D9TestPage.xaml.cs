namespace LCDHardwareMonitor.Presentation.Views
{
	...
	using POINT=LCDHardwareMonitor.Renderers.D3D9Renderer.POINT;
	...
	public partial class D3D9TestPage : UserControl
	{
		...
		public D3D9TestPage ()
		{
			...
			HRESULT.Check(D3D9Renderer.SetSize(512, 512));
			HRESULT.Check(D3D9Renderer.SetAlpha(false));
			HRESULT.Check(D3D9Renderer.SetNumDesiredSamples(4));
		...
		~D3D9TestPage()
		{
			D3D9Renderer.Destroy();
		}

		void AdapterTimer_Tick(object sender, EventArgs e)
		{
			Point screenPos = imgelt.PointToScreen(new Point(0, 0));
			POINT p = new POINT((int) screenPos.X, (int) screenPos.Y);

			HRESULT.Check(D3D9Renderer.SetAdapter(p));
		}
		...
		void SizeTimer_Tick(object sender, EventArgs e)
		{
			...
			{
				HRESULT.Check(D3D9Renderer.SetSize(actualWidth, actualHeight));
			}
		}
		...
		void CompositionTarget_Rendering(object sender, EventArgs e)
		{
			...
			if (d3dimg.IsFrontBufferAvailable && _lastRender != args.RenderingTime)
			{
				...
				HRESULT.Check(D3D9Renderer.GetBackBufferNoRef(out pSurface));
				if (pSurface != IntPtr.Zero)
				{
					...
					HRESULT.Check(D3D9Renderer.Render());
